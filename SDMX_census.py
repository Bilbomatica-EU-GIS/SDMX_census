...
def SDMX2df():
    ...
    if df['AREA_OF_DISSEMINATION'].iat[0] == 'EU':
        ...
        df['OBS_STATUS'] = df['OBS_STATUS'].str.strip()
        df['Flag'] = 1
        df.to_csv(r'C:\Users\hecto\Desktop\Projects\GISCO-SDMX\svn-new-project\SMDX_census\INT\df_groupby_spatial.csv')
        if outputfile == 'gpkg':
            duplicateSTAT(df)
        elif outputfile == 'SDMX':
            df.drop(columns=['SPATIAL'],inplace= True)
            df.rename(columns={'GRD_ID':'SPATIAL'},inplace= True)
            df.to_csv(r'C:\Users\hecto\Desktop\Projects\GISCO-SDMX\svn-new-project\SMDX_census\INT\df_groupby_spatial2.csv')
            CSV2SDMX(df)
    else:
        df['Flag'] = 0
        duplicateSTAT(df)
...
def duplicateSTAT(df):
    ...
    df.to_csv(r'C:\Users\hecto\Desktop\Projects\GISCO-SDMX\svn-new-project\SMDX_census\INT\df_duplicateSTAT.csv')
